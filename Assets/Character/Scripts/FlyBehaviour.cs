...
public class FlyBehaviour : GenericBehaviour{
	public bool		flyBehaviourEnable;
	...
	void Update(){
		...
		if (flyBehaviourEnable && Input.GetButtonDown(flyButton) && !behaviourManager.IsOverriding() && !behaviourManager.GetTempLockStatus(behaviourManager.GetDefaultBehaviour)){
			fly = !fly;
